...
for (const user of users) {
  ...
  if (hasOwn(data, "stripeStatus")) continue;
  ...
  missing.push({
    ...
    createTime: formatTimestamp(user.createTime),
    stripeCustomerId: data.stripeCustomerId || "",
    stripeSubscriptionId: data.stripeSubscriptionId || "",
    stripeSubscriptionStatus: data.stripeSubscriptionStatus || "",
    stripeLastPayment: data.stripeLastPayment || "",
    stripeCheckoutSessionId: data.stripeCheckoutSessionId || "",
  });
}

missing.sort((a, b) => (a.createdAt || a.createTime).localeCompare(b.createdAt || b.createTime));

console.log(`Total users: ${users.length}`);
console.log(`Users missing stripeStatus: ${missing.length}`);
